...
2024-03-21 - Unbounded Settings File Read\n\nVulnerability: Reading local files with a large upper bound (like 2MB) or unbounded reads can cause memory exhaustion, leading to a local Denial of Service (DoS) attack on constrained systems.\nLearning: To prevent DoS attacks via memory exhaustion from maliciously large local configuration files, always use a strict, small bound for reading based on expected file size (e.g., 64KB for simple JSON settings).\nPrevention: Use bounded reads via `std::io::Read::read_to_string` and `.take(limit)` with small, tailored limits for local configurations.

## 2025-03-03 - Insecure Default URLs Allowed in CLI and Environment Variables

**Vulnerability:** The application allowed users and environments to override critical API endpoints (`forecast_url`, `air_quality_url`, `formula_url`) using plain `http://` protocols via CLI arguments or environment variables. This creates a Man-in-the-Middle (MitM) risk where an attacker intercepting the network traffic could alter weather data or trick the update check mechanism.
**Learning:** Even when exposing configuration options to users or environments, the application must enforce strict security baselines. Network communications that retrieve updates or external data must be verified to use encrypted protocols to protect integrity and confidentiality.
**Prevention:** Filtered dynamically supplied URLs during loading and CLI parsing to ensure they strictly start with `https://` (while explicitly allowing `http://localhost` and `http://127.0.0.1` for local testing/development) and fallback securely to known-good hardcoded endpoints if invalid protocols are provided.
